...
namespace Vkbase {
RenderPass::RenderPass(const std::string &resourceName,
...
const Framebuffer &RenderPass::createFramebuffer(
  ...
  return *(new Framebuffer(resourceName, _device.name(), name(),
                           attachmentNames, width, height));
}

const Pipeline &RenderPass::createPipeline(
    const std::string &resourceName,
    const PipelineCreateInfo &createInfo) const {
  return *(new Pipeline(resourceName, _device.name(), name(), createInfo));
}

void RenderPass::begin(const vk::CommandBuffer &commandBuffer,
