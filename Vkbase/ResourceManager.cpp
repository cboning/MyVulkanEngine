...
void ResourceManager::createInstance(std::vector<const char *> layers,
    ...
    std::vector<vk::LayerProperties> usableLayers =
    ...
    for (const char *layerName : layers)
        if (std::find(usableLayerNames.begin(), usableLayerNames.end(),
                                    layerName) != usableLayerNames.end())
...
void ResourceManager::addResource(ResourceBase *pResource) {
    std::unordered_map<std::string, ResourceBase *> &resources =
            _pResources[pResource->type()];
    std::unordered_map<std::string, ResourceBase *>::iterator iter =
            resources.find(pResource->name());
    if (iter != resources.end()) {
        if (iter->second == pResource) return;
#ifdef DEBUG
        std::cout << "[Warning] Resource with Type: " << toString(pResource->type())
                            << ", Name: " << pResource->name()
...
ResourceBase *ResourceManager::resource(ResourceType type,
                                                                                std::string name) const {
    Vkbase::ResourceSet::const_iterator typeIter = _pResources.find(type);
    if (typeIter == _pResources.end()) return nullptr;
    const std::unordered_map<std::string, ResourceBase *> &resources =
            typeIter->second;
    const std::unordered_map<std::string, ResourceBase *>::const_iterator iter =
            resources.find(name);
    if (iter != resources.end())
        return iter->second;
    else
...
void ResourceManager::remove(ResourceType type, std::string name) {
    ...
    std::unordered_map<std::string, Vkbase::ResourceBase *>::iterator iter =
            _pResources[type].find(name);
    ...
    if (_pResources[type].empty()) _pResources.erase(type);
}

void ResourceManager::renameResource(const ResourceBase *pResource,
    ...
    if (pTempResource) pTempResource->destroy();
