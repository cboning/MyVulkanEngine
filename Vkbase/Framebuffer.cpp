...
namespace Vkbase
{
    Framebuffer::Framebuffer(const std::string &resourceName, const std::string &deviceName, const std::string &renderPassName, const std::vector<const std::string> &attachmentNames, uint32_t width, uint32_t height)
        : ResourceBase(Vkbase::ResourceType::Framebuffer, resourceName), _device(*dynamic_cast<const Device *>(connectTo(resourceManager().resource(Vkbase::ResourceType::Device, deviceName)))), _renderPass(*dynamic_cast<const RenderPass *>(connectTo(resourceManager().resource(Vkbase::ResourceType::RenderPass, renderPassName))))
    {
