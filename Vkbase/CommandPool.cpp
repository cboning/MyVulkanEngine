...
CommandPool::CommandPool(const std::string &resourceName, const std::string &deviceName, CommandPoolQueueType queueType)
    : VkResourceBase(VkResourceType::CommandPool, resourceName),
      _device(*dynamic_cast<const Device *>(connectTo(resourceManager().resource(VkResourceType::Device, deviceName)))),
...
CommandPool::~CommandPool() { _device.device().destroyCommandPool(_commandPool); }
