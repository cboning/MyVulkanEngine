...
RenderObjectDelegator::RenderObjectDelegator(const std::string &deviceName, const Camera &camera, uint32_t flightFrameCount, vk::DeviceSize uboSize)
    : _flightFrameCount(flightFrameCount), _descriptorSetsName(createResource<DescriptorSets>("", deviceName)->name()), _deviceName(deviceName),
      _pCamera(&camera)
{
    createUBOs(uboSize);
}
...
void RenderObjectDelegator::draw(CommandBuffer *pCommandBuffer, const std::string &renderPassName, const std::string &pipelineName, uint32_t imageIndex,
